...
%%
first_ring=reshape(qCompTree_3Ls1.surf0(1,:,:),[size(qCompTree_3Ls1.surf0,1),3]);
center1=mean(first_ring,1);
qCompTree_3Ls1.surf0=qCompTree_3Ls1.surf0-reshape(center1,1,1,3);

first_ring=reshape(qCompTree_3Ls2.surf0(1,:,:),[size(qCompTree_3Ls2.surf0,1),3]);
center2=mean(first_ring,1);
surf2=qCompTree_3Ls2.surf0-reshape(center2,1,1,3);

[qCompTree_3Ls1p.surf0]=global_reparm(qCompTree_3Ls1.surf0,surf2,qCompTree_3Ls1.srnf0,qCompTree_3Ls2.srnf0);

qCompTree_3Ls1p.surf0=qCompTree_3Ls1p.surf0+reshape(center1,1,1,3);


isAllZero = all(qCompTree_3Ls1p.surf0(:) == 0);

if isAllZero
    qCompTree_3Ls1p.srnf0 = zeros(size(qCompTree_3Ls1p.surf0,1),size(qCompTree_3Ls1p.surf0,2),size(qCompTree_3Ls1p.surf0,3));
    
else
    [Anew,normal_new,multfactnew,sqrtmultfactnew] = area_surf(qCompTree_3Ls1p.surf0);
    qCompTree_3Ls1p.srnf0 = surface_to_q(normal_new,sqrtmultfactnew);
 
end
% q0 q-curve diff
[gam0, Emain] = DPQ_difflen(qCompTree_3Ls1p.srnf0, qCompTree_3Ls2.srnf0,qCompTree_3Ls1p.surf0, qCompTree_3Ls2.surf0);  % --- DPQ_difflen computes the difference between two q-space branch.
...
qCompTree_3Ls1p.surf0=Apply_Gamma_Surf(qCompTree_3Ls1p.surf0,gam0,size(qCompTree_3Ls1p.surf0,1));
...
qCompTree_3Ls1p.beta0_rad =  qCompTree_3Ls1.beta0_rad;

%% s, len0
qCompTree_3Ls1p.s = qCompTree_3Ls1.s;
...
qCompTree_3Ls1p.surf= cell(1, K);
qCompTree_3Ls1p.srnf= cell(1, K);
...
for i=1:m
    ...
    qCompTree_3Ls1p.q_children{matched(i,2)}.surf0=qCompTree_3Ls1.surf{matched(i,1)};
    
    qCompTree_3Ls1p.q_children{matched(i,2)}.srnf0=qCompTree_3Ls1.srnf{matched(i,1)};

    qCompTree_3Ls1p.q{matched(i,2)} = qCompTree_3Ls1.q{matched(i,1)};
    qCompTree_3Ls1p.surf{matched(i,2)} = qCompTree_3Ls1.surf{matched(i,1)};
    qCompTree_3Ls1p.srnf{matched(i,2)} = qCompTree_3Ls1.srnf{matched(i,1)};
...
%%% tk, sk
qCompTree_3Ls1p.tk_sideLocs = qCompTree_3Ls1.tk_sideLocs;%zeros(1,K);
qCompTree_3Ls1p.sk = zeros(1,K);

% matched,unmatched1 just get updated by gam0
tk1p =qCompTree_3Ls1p.tk_sideLocs;% interp1(G.gam0, qCompTree_3Ls1p.t_paras, qCompTree_3Ls1.tk_sideLocs);
