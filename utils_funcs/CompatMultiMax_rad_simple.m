...
surf0_all = cellfun(@(A)A.surf0, Qs,'UniformOutput', false);
srnf0_all = cellfun(@(A)A.srnf0, Qs,'UniformOutput', false);
...
% side branches
[q_s,surf, srnf] = cellfun(@(A) ReCalc_q(A, K_max, q_simple_max),...
                            Qs, 'UniformOutput', false);
...
% Qs = cell(1, length(Qs));
for i=1: length(Qs)
    Qs{i} = make_qST_rad(q0s{i},srnf0_all{i},surf0_all{i}, q_s{i}, srnf{i},surf{i},sk_s{i}, Qs{i}.b00_startP, ...
...
function [q_s,surf,srnf]= ReCalc_q(Q, K_max, q_simple_max)
...
if K < K_max
    for i=K+1:K_max
        ...
        surf{i}= zeros(30,30,3);
        srnf{i}= zeros(30,30,3);
    end
end
