function [qX,sX,qsX] = flattenQCompTree_3layers_rad(qCompTree_3Ls, lam_m, lam_s, lam_p, qX, sX, qsX,locator_s,locator_s0,locator_qs0)
...
sX(locator_s0:locator_s1-1) = reshape(qCompTree_3Ls.surf0,[1,ds]);
isAllZeroq1 = all(qCompTree_3Ls.surf0(:) == 0);   
if isAllZeroq1
      Q1_surf= zeros(T0,T0,6);
      Lx1=zeros(T0,T0);
      Ly1=zeros(T0,T0);
else
    [Q1_surf,Lx1,Ly1]=surface_to_tangent(qCompTree_3Ls.surf0);
end
...
qsX(locator_qs0:locator_qs1-1) = reshape(qCompTree_3Ls.surf0(:,1,:),[1,T0*d]);
...
locator_s1 = locator_s1 + numel(qCompTree_3Ls.q_children);

locator_qs0 = locator_qs1;
locator_qs1 = locator_qs1 + numel(qCompTree_3Ls.q_children);

locator_s = locator_e;
locator_e = locator_s + numel(qCompTree_3Ls.q_children);

qX(locator_s:locator_e -1) = sqrt(lam_p)*qCompTree_3Ls.sk(1:end);
sX(locator_s0:locator_s1 -1) = sqrt(lam_p)*qCompTree_3Ls.sk(1:end);
qsX(locator_qs0:locator_qs1 -1) = sqrt(lam_p)*qCompTree_3Ls.sk(1:end);
%fprintf('3Ls%d-', locator_e-locator_s);

%getting medial axis
locator_qs0 = locator_qs1;
locator_qs1 = locator_qs1 + (T0*d);
qsX(locator_qs0:locator_qs1-1) = reshape(qCompTree_3Ls.q0,[1,T0*d]);
...
for k=1: numel(qCompTree_3Ls.q_children)
    ...
    [qX,sX,qsX] = flattenQSimpleTree_rad(qCompTree_3Ls.q_children{k}, lam_m, lam_s, lam_p, qX, sX, qsX,locator_s,locator_s0,locator_qs0);
